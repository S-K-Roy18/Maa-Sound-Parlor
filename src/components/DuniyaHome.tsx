...
export function DuniyaHome({ onSelectWorld }: DuniyaHomeProps) {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center pt-16 sm:pt-24 px-4 sm:px-8 md:px-12 pb-32 relative z-10 overflow-x-hidden perspective-1000">
      ...
      >
        <h1 className="text-[4rem] sm:text-[6rem] md:text-[8rem] lg:text-[10rem] xl:text-[12rem] leading-none font-hindi text-white mb-4 md:mb-6 text-cinematic tracking-tight wrap-break-word text-center">
          माँ साउंड पार्लर
        </h1>

        <div className="flex items-center justify-center gap-4 sm:gap-8 text-white/50 mb-8 sm:mb-12 w-full max-w-sm sm:max-w-md mx-auto px-4">
          <motion.div 
            initial={{ flexGrow: 0 }} animate={{ flexGrow: 1 }} transition={{ duration: 1, delay: 1 }}
            className="h-px bg-linear-to-r from-transparent to-white"
          />
          <motion.span 
            initial={{ opacity: 0, letterSpacing: "0em" }} 
            animate={{ opacity: 1, letterSpacing: "0.5em" }} 
            transition={{ duration: 1.5, delay: 1 }}
            className="text-base sm:text-xl font-light pl-2" // pl-2 to offset the tracking
          >
            DUNIYA
          </motion.span>
          <motion.div 
            initial={{ flexGrow: 0 }} animate={{ flexGrow: 1 }} transition={{ duration: 1, delay: 1 }}
            className="h-px bg-linear-to-l from-transparent to-white"
          />
        </div>
      ...
      <motion.div 
        ...
        initial="hidden"
        animate="show"
        className="flex flex-wrap justify-center gap-6 max-w-375 w-full mx-auto"
